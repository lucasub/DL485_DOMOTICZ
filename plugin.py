...
config_file_name = PATH + "/config.json"  # File di configurazione
...
""" LOOP """
class BasePlugin(Bus):
    def __init__(self):
        super().__init__(config_file_name)
        self.bus_dl485 = BusDL485('log.txt', logstate)
        self.debug = 0
        ...
        self.system = 'Domoticz' # Indica alla classe chi la stà istanziando
    ...
    def onStart(self):
        ...
        for board_id in self.mapiotype: # Iterazione di tutte le board su config.json
            ...
            for logic_io in self.mapiotype[board_id]: # iterazione per ogni logic_io
                ...
                dtype = self.mapiotype[board_id][logic_io]['dtype']
                ...
                if dtype == 'switch':
                    ...
                    options = {'Custom': '1;{}'.format(self.mapiotype[board_id][logic_io]['dunit'])}
                    sValue = "0"
    ...
    def updateIO(self, board_id, logic_io, value):
        ...
        if (Unit in Devices):
            ...
            if dtype == 'Switch':
                ...
                # x_value = value[0]
                # value = value[0]
                # value = self.make_inverted(board_id, logic_io, value[0] & 1)  # Inverte l'IO se definito sul file di configurazione
                ...
                linked_proc = self.mapproc[DeviceID] if DeviceID in self.mapproc else {}
                ...
                #             if x_value & 0b100:
                #                 value = self.status[x_board_id]['io'][x_logic_io - 1]
                #                 value = 1 - value

                #                 value = self.make_inverted(x_board_id, x_logic_io, value)  # Inverte l'IO se definito sul file di configurazione
                #                 # print("TOGGLE", x_out, value)
                #                 msg = self.writeIO(x_board_id, x_logic_io, [value])
                ...
                #             linked_board_id_logic_io = self.mapiotype[x_board_id][x_logic_io]['linked_board_id_logic_io']
                ...
                #                 value = value & self.status[ii_board_id]['io'][ii_logic_io - 1]
                #             value = self.make_inverted(x_board_id, x_logic_io, value)  # Inverte l'IO se definito sul file di configurazione
                #             msg = self.writeIO(x_board_id, x_logic_io, [value])
                #             self.TXmsg.append(msg)
                ...
                if value:
                    ...
                    if bio in self.mapproc:
                        ...
                        if device_type_linked =="PSYCHROMETER":
                            value_humidity = self.status[board_id_linked]['io'][logic_io_linked - 1]
                            ...
                            if value_humidity:
                                ...
                                # dtypeLinked = self.mapiotype[board_id_linked][logic_io_linked]['dtype']
                ...
                # value = self.calculate(board_id, logic_io, value)
    ...
    def onCommand(self, Unit, Command, Level, Hue):
        ...
        if Command == 'Off':
            ...
            value = 1
            if "plc_function" in self.mapiotype[board_id][logic_io] and self.mapiotype[board_id][logic_io]["plc_function"] == 'dimmer':
            ...
            for k in Hue:
                if k == 'r':
                    print("RED:", Hue[k])
                    msg = self.writeIO(board_id, 1, [Hue[k]])
                    self.TXmsg.append(msg)
                elif k == 'g':
                    print("GREEN:", Hue[k])
                    msg = self.writeIO(board_id, 2, [Hue[k]])
                    self.TXmsg.append(msg)
                elif k == 'b':
                    print("BLUE:", Hue[k])
                    msg = self.writeIO(board_id, 3, [Hue[k]])
                    self.TXmsg.append(msg)
            value = Level
        else:
    ...
    def onMessage(self, Connection, RXbytes):
        for d in RXbytes:
            ...
            if len(self.RXtrama) > 1:  # ERA 1 Mosra solo comunicazioni valide (senza PING)
                # print("Trama arrivata:", self.RXtrama, self.int2hex(self.RXtrama))
                ...
                if self.RXtrama[1] in [ self.code['COMUNICA_IO'], self.code['RFID'] ]:  # COMUNICA_IO / Scrive valore USCITA
                    ...
                    # print("VALUE::", value)
                    self.updateIO(self.RXtrama[0], self.RXtrama[2], value)  # Aggiorna DOMOTICZ

                    # print("VALUE CALCULATE:", self.RXtrama[0], self.RXtrama[2], self.RXtrama[3:], value)
                elif self.RXtrama[1] == self.code['CR_GET_BOARD_TYPE'] | 32:
                    # print("===>>>>> GET BOARD ID", self.getBoardType)
                    pass
                else:
                    pass
                    # print("{:<7}  NO COMUNICA IO         {}".format(self.nowtime, self.RXtrama))
